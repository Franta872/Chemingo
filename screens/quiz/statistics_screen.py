...
class StatisticsScreen(ModalScreen):
    def compose(self) -> ComposeResult:
        ...
        with Container(id="statistics-container"):
            ...
            tree.root.add_leaf(t("remaining_questions", "quiz")+": "+("♾️" if isinf(state.num_of_questions) else f"[b]{state.num_of_questions}[/]")) # type: ignore[attr-defined]

            chosen_items = tree.root.add_leaf(t("chosen_items", "quiz")+f": [b]{len(state.selected_elements)+count_dictionary_list_items(state.selected_compounds)}[/]") # type: ignore[attr-defined]
            chosen_items.add_leaf(t("chosen_elements", "quiz")+f": [b]{len(state.selected_elements)}[/]") # type: ignore[attr-defined]
            chosen_items.add_leaf(t("chosen_compounds", "quiz")+f": [b]{count_dictionary_list_items(state.selected_compounds)}[/]") # type: ignore[attr-defined]
            ...
            boolean_question.add_leaf(f"{t("enabled", "quiz")}: [b]{t("yes", "choice") if state.question_answers["boolean"] else t("no", "choice")}[/]") # type: ignore[attr-defined]
            ...
            choice_question.add_leaf(f"{t("enabled", "quiz")}: [b]{t("yes", "choice") if state.question_answers["choice"] else t("no", "choice")}[/]") # type: ignore[attr-defined]
